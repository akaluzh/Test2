create or replace package PAC_CLAIM is
  ...
  procedure p_Cancel_Claim (vClaim number, vOrg number);
  procedure p_Cancel_ServiceReserve (vService number, vOrg number);
    ...
    if vActType in ('add_service', 'ns', 'replace_service') then 
        ...
        if vNum is null then
          ...
          update claims cl set cl.zone_new = vZoneCode, cl.num_new = vNum
             where cl.id = vClaim;
          update service s set s.code = vNum, s.phone_number = vNum, s.zonecode = vZoneCode, s.reserve_date = vReserveDate
           where s.id = vservice;   
        end if;
  ...
  begin
      ...
      select w.claim, w.cross_type, lower(a.acronym), w.load_type, act.acronym 
        ...
        from workorder w join  
             claims cl on w.claim = cl.id join  
      ...
      -- для услуг телефонии - кроссирование
      if vCrossType = 0 and vCrossDir = 'cross' and vActivityType <> 'connect' then
         ...
         -- изменяем статус сервиса          
          ...
          update service s 
             set s.status = f_get_abstypes_id ('service_status', 'enabled')
           where s.id = vService;
      end if; 
        
            -- для услуг телефонии - кроссирование, временное подключение
      if vCrossType = 0 and vCrossDir = 'cross' and vActivityType = 'connect' then

         -- изменяем статус сервиса
\*          select s.id into vservice
            from service s join 
                 type_of_service tos on s.type_of_service = tos.id join 
                 claims cl on (s.phone_number = cl.num_old and s.zonecode = cl.zone_old )
           where tos.code in ('Indiv', 'Coupled') 
             and s.status = (select a.id from abs_types a where a.meta_cathegory = 'SERVICE_STATUS' and a.acronym = 'reserved_old')
             and cl.id = vClaim ;*\
      ...
      -- для услуг телефонии - раскроссирование      
      if vCrossType = 0 and vCrossDir = 'uncross' then
...
\*          select s.id into vservice
         ...
         select w.service into vService
           from workorder w where w.id = vWorkorder;    
             
         if vActivityType = 'replace_number' then
           delete from service s where s.id = vService;
         else
           update service s 
              set s.status = f_get_abstypes_id ('service_status', 'reserved_old') 
            where s.id = vService;
         end if;    
      ...
      if vCrossType = 1 then
          if vCrossDir = 'cross' then
            update service s 
               set s.status = f_get_abstypes_id ('service_status', 'enabled') 
             where s.id = vService;  
                       
          elsif vCrossDir = 'uncross' then
            select s.id into vService 
              from service s 
              where s.type_of_service = f_get_ServType('DialupDSL')
                and s.full_number = (select wo.phone_num from workorder wo where wo.id = vWorkorder) ;

            p_DisConnect_Dslam(vService);
          end if;  
      end if;     
...
--------------------------------------------  
  ...
  begin
      ...
      select w.organization, w.claim, w.cross_type, lower(a.acronym), w.load_type, act.acronym 
        into vOrg, vClaim, vCrossType, vCrossDir, vLoadType, vActivityType
        from workorder w left join  
             claims cl on w.claim = cl.id left join  
             abs_types act on cl.activity_type = act.id join
             abs_types a on w.cross_direction = a.id where w.id = vWorkorder;

         -- изменяем статус наряда 
      update workorder w 
         set w.status = f_get_abstypes_id ('workorder_status', 'done'),
             w.status_details = f_get_abstypes_id ('claim_results', '0'),
             w.date_to = sysdate
       where w.id = vWorkorder;  
         ...
         -- изменяем статус сервиса      
      select w.service, s.full_number into vService, vFullNumber 
        from workorder w join service s on w.service = s.id where w.id = vWorkorder;    
     
      update service s 
             set s.status = decode(vCrossDir, 
                                   'cross', 
                                   f_get_abstypes_id ('service_status', 'enabled'), 
                                   f_get_abstypes_id ('service_status', 'disabled'))
           where s.id = vService;
      ...
      if vid = 0 then
         ...
         -- если наряд на поделючение adsl - достраиваем трассу сервиса телефонии
      if vCrossType = 1 and vCrossDir = 'cross' then
        select s.id into vPhoneService
          from service s 
         where s.organization = vOrg and s.type_of_service = f_get_ServType('Indiv') and s.full_number = vFullNumber;
        
        for k in (select cs.connector, c.contact_cnt_from, c.contact_from, c.contact_cnt_to, c.contact_to
                    from connector_service cs join 
                         connector c on cs.connector = c.id 
                   where cs.service = vService)
          loop
               select count(1) into vid 
                from connector_service ct where ct.connector = k.connector and ct.service = vPhoneService;
              if vid = 0 then
                insert into connector_service(connector, service, organization) 
                  values (k.connector, vPhoneService, vOrg);
              end if; 
              
              select count(1) into vid 
                from contact_service ct 
               where ct.contact_cnt = k.contact_cnt_from and ct.contact = k.contact_from and ct.service = vPhoneService;
              if vid = 0 then
                insert into contact_service (service, contact_cnt, contact, organization, sort_order) 
                  select vPhoneService, k.contact_cnt_from, k.contact_from, vOrg, cs.sort_order
                    from contact_service cs 
                   where cs.contact_cnt = k.contact_cnt_from and cs.contact = k.contact_from and cs.service = vService;
              end if; 
              
              select count(1) into vid 
                from contact_service ct 
               where ct.contact_cnt = k.contact_cnt_to and ct.contact = k.contact_to and ct.service = vPhoneService;
              if vid = 0 then
                insert into contact_service (service, contact_cnt, contact, organization, sort_order) 
                  select vPhoneService, k.contact_cnt_to, k.contact_to, vOrg, cs.sort_order
                    from contact_service cs 
                   where cs.contact_cnt = k.contact_cnt_to and cs.contact = k.contact_to and cs.service = vService;
              end if;              
          end loop;
        
         pac_service_trace.p_Sort_ConnectorService(vPhoneService, vOrg); 
...
-------------------------------------------------------------  
  ...
  procedure p_Connect_Dslam1(vLC number, vLC_contact number, vNum number, vServiceADSL out number) is
    vOrg           number;
    vid            number;
    vService       number;
    vPhoneStrafe   number;
...
---------------------------------------------------------------------------
 /* procedure bulk_reconnection (vid number) is 
  ...
  begin
   ...
   select lch.id, lc.id, ct.id, lch.site into vChassis, vCard, vContact, vSite
  ...
  select srv.full_number into vPhoneNum
    from cross cr join
         strafe s on cr.id = s.cross_id join
         contact_service cs1 on s.id = cs1.contact_cnt join
         service srv on cs1.service = srv.id 
   where cr.site_id = vSite
     and srv.type_of_service = 41785201
     and not exists (select id from service srv1 
                      where srv.full_number = srv1.full_number 
                        and srv1.type_of_service <> 41785201
                        and srv1.organization = vOrg)
     and rownum = 1;
  ...
  procedure p_Cancel_Claim (vClaim number, vOrg number) is
    vService   number;
  begin
  
    select cl.service_new into vService
      from claims cl where cl.id = vClaim and cl.organization = vOrg;
      
    p_Cancel_ServiceReserve (vService, vOrg);
      
    delete from claim_trace t where t.claim = vClaim;
      
    update claims cl
       set cl.status = (select id from abs_types a where a.meta_cathegory = 'CLAIM_STATUS' and a.acronym = 'canceled'),
           cl.end_date = trunc(sysdate),
           cl.status_details = null
     where cl.id = vClaim;    
  ...
  procedure p_Cancel_ServiceReserve (vService number, vOrg number) is
    vServStatus   varchar2(50);
    vid           number;
  begin
    
    select a.acronym into vServStatus
      from service s join 
           abs_types a on s.status = a.id 
     where s.id = vService and s.organization = vOrg;
     
    if vServStatus <> 'reserved' then
      raise_application_error (-20001, 'Сервіс повинен мати статус "Зарезервований"');
    end if;
     
    for k in (select cs.id, cn.id as connector, a.code as ConnType 
                    from connector_service cs join
                         connector cn on cs.connector = cn.id left join
                         abs_types a on cn.type = a.id
                   where cs.service = vService)
      loop
        delete from connector_service t where t.id = k.id;
            
        if k.ConnType = 'single' then
           select count(1) into vid from connector_service sv where sv.connector = k.connector;
              
          if vid = 0 then
              delete from connector t where t.id = k.connector; 
          end if;
        end if;
      end loop;  
...
--  добавляем ДСЛАМ-участок                    
    ...
    for c in (select tt.*
     ...
     loop
        select count(1) into vid 
          from contact_service sc 
         where sc.contact_cnt = c.contact_cnt1 and sc.contact = c.contact1 and sc.service = vServiceADSL and sc.organization = vOrg;
        if vid = 0 then
          insert into contact_service (contact_cnt, contact, service, organization, sort_order)
                 values (c.contact_cnt1, c.contact1, vServiceADSL, vOrg, i);
          i := i+1;       
        end if;         
        
        select count(1) into vid 
          from contact_service sc 
         where sc.contact_cnt = c.contact_cnt2 and sc.contact = c.contact2 and sc.service = vServiceADSL and sc.organization = vOrg;
        if vid = 0 then
          insert into contact_service (contact_cnt, contact, service, organization, sort_order)
                 values (c.contact_cnt2, c.contact2, vServiceADSL, vOrg, i);
          i := i+1;       
        end if;
                
        select count(1) into vid 
          from connector_service sc 
         where sc.connector = c.id and sc.service = vServiceADSL and sc.organization = vOrg;
        if vid = 0 then
          insert into connector_service (connector, service, organization)
                 values (c.id, vServiceADSL, vOrg);
        end if;         
     end loop;      
    ...
    begin
      select c.contact_cnt_to, c.contact_to into vContact_cnt, vContact
        from connector_service cs join
             connector c on cs.connector = c.id join
             contact c1 on c.contact_from = c1.id 
       where cs.service =  vServiceADSL and c.contact_cnt_from <> c.contact_cnt_to 
         and c1.contact_type = decode(vOrgRegion, 26, 'dsl', 'combo');
    exception
      when no_data_found then 
        select c.contact_cnt_from, c.contact_from into vContact_cnt, vContact
    ...
    insert into connector_service (connector, service, organization, sort_order)
                 values (vConnector, vServiceADSL, vOrg, i);
                 
    update service s set s.contact_cnt_end = vLC, s.contact_end = vLC_contact
     where s.id = vServiceADSL;
     
    pac_service_trace.p_Sort_ConnectorService(vServiceADSL, vOrg); 
